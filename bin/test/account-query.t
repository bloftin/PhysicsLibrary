...
for my $file (
    ...
    ['Password.pm', qw(pwChange changePassword pwChangeRequest)],
...
subtest 'recovery lookup and change' => sub {
    fixture(); @results = ({username => "O'Neil", email => 'member@example.invalid'});
    is(Noosphere::pwChangeRequest({submit => 1, username => "o'neil"}), 'Mail Sent', 'recovery request');
    is($queries[0]->{sql}, 'SELECT username, email FROM users WHERE username = ? LIMIT 1', 'fixed lookup SQL');
    ...
    for my $password (undef, [], '') {
        fixture();
        like(Noosphere::changePassword($hash, $password), qr/enter a password/, 'invalid password rejected');
        is(scalar @queries, 0, 'invalid password cannot write');
    }
    fixture(); @results = ('0E0');
    like(Noosphere::changePassword($hash, 'new'), qr/Could not change/, 'zero affected rows not reported as success');
    fixture();
    like(Noosphere::pwChange({hash => $hash, submit => 1, pw1 => 'one', pw2 => 'two'}),
        qr/passwords don't match/, 'confirmation mismatch does not save');
    is(scalar @queries, 0, 'mismatch never reaches database');
};
...
subtest 'database errors are closed and redacted' => sub {
    ...
    for my $mode (qw(prepare prepare-undef execute execute-undef)) {
        ...
        my $html = Noosphere::changePassword($hash, $quoted_password);
