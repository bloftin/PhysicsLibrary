...
sub closeCorrection {
	...
	if ($directive eq 'accept' && !hasPermissionTo(getConfig('en_tbl'),$params->{id},{uid=>$cor{userid}},'write')) {
...
sub postCorrection {
	...
	if (defined $params->{post}) {
		...
		if (blank($error)) {
			...
			return paddingTable(clearBox("Correction filed","Thanks for submitting a correction. You can go back to the parent object by clicking <a href=\"".getConfig("main_url")."/?op=getobj&amp;from=$table&amp;id=$params->{id}\">here</a>."));
		}
		else {
