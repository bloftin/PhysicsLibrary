...
sub siteDocOld {
	...
	# that are site docs
	#
	my $q = "select * from $collab where sitedoc=1";
	$sth = $dbh->prepare($q);
	$sth->execute();

	while (my $row = $sth->fetchrow_hashref()) {

		$template->addText('	<docitem>');

		$template->addText("		<uid>$row->{uid}</uid>");
		$template->addText("		<title>$row->{title}</title>");

		if (defined $row->{'abstract'}) {
			$template->addText("		<abstract>$row->{abstract}</abstract>");
		}
=quote
...
=cut
		...
		$template->addText("		<owner>1</owner>") if $row->{'userid'} == $userinf->{'uid'};
...
sub renderCollab {
	...
	#
	my $content = getRenderedContentHtml($table, $rec, $method);
...
=quote
	...
	if (!$params->{'new'}) {
		return errorMessage("You dont have permission to edit that object!") unless (hasPermissionTo($params->{'from'},$params->{'id'},$userinf,'write'));
...
sub renderCollabPreview {
	...
	my $table = getConfig('collab_tbl');
	my $id = defined($params->{'id'}) && $params->{'id'} ne '' ? $params->{'id'} : $dir;
	my ($latex, $links) = prepareCollabForRendering(
		$params->{'data'},
		$method,
		$params->{'title'},
		$table,
