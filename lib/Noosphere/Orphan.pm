...
sub _changeObjectOwner {
	...
	#
	# Ben - update to unix_timestamp like the newest noosphere
        #setfields(getConfig('cor_tbl'),"graceint = now() - filed","objectid = $objectid and closed is null") if $tbl eq getConfig('en_tbl');
        setfields(getConfig('cor_tbl'),"graceint = unix_timestamp(now()) - unix_timestamp(filed)","objectid = $objectid and closed is null");
	# update owner log
	#
