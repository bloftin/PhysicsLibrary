...
# translate ampersands for URLs in PlanetPhysics entry bodies
#
sub protectURL {
	my $url = shift;
...
sub stdmsg {
	...
	# must do this after hyperlinking, otherwise spaces will look like &nbsp;
	#
	...
	$message =~ s/((http|ftp):\/\/(\.$lcc|$lcc)+)/<a href="$1">$1<\/a>/ig;
...
sub tohtmlascii {
	...
	$msg =~ s/\n/<br \/>/g;			# \n to <br>
	$msg =~ s/ /\&nbsp;/g;			# turn spaces to &nbsp;
	$msg =~ s/(?<!\&nbsp;)\&nbsp;/$1 /g;	# put back seperator spaces
...
#
sub objectExistsByUid {
	my $uid = shift;
	my $table = shift || getConfig('en_tbl');

	my ($rv,$sth) = dbSelect($dbh,{WHAT=>'uid',FROM=>$table,WHERE=>"uid=$uid"});
