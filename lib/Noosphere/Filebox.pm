...
sub cacheFileBox {
	...
	mkdir "$cacheroot/$table/$id" if (not -e "$cacheroot/$table/$id");
	mkdir "$cacheroot/$table/$id/$method" if (not -e "$cacheroot/$table/$id/$method");

	my @files = <$fileroot/$table/$id/*>;
	for my $file (@files) {
		`cp $file $cacheroot/$table/$id/$method`;
	}
}
