...
# Ben :modified not sure why it did not work
#  return $dbh = DBI->connect("DBI:".'Pg'.":dbname=".getConfig('db_name').";",getConfig('db_user'), getConfig('db_pass'));
...
sub dbGather {
	...
	foreach my $table (keys %idlists) {
		...
		#dwarn "doing gather query = $query", 2;
