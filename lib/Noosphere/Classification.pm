...
sub printclass {
	...
	foreach my $row (@class) {
		...
		if ($curns ne $row->{ns}) {
			$nsprintable =~ s/ /&nbsp;/;
			if (nb($nslink)) {
				$html .= "<td valign=\"top\"><font size=\"$fs\"><a target=\"physicslibrary_popup\" href=\"".getConfig("main_url")."/?op=pacsbrowse\">$nsprintable</a>:&nbsp;</font></td>";
			} else {
...
# the classification strnig should be of the form
...
sub normalizecat {
	...
	#
	if ($cat!~/:/) {
		($ns,$catstring) = (getConfig('default_ns'),$cat);
	} else {
