...
void connection::send_query_score(const query_score& score) {

	char buf[BUFFSIZE+1] = {0};

		// send the query
		//
		for( int i = 0; i < score.query.size(); ++i) {

		  // write a \n after the last clause
		  //
		  if (i == score.query.size() - 1) {
			snprintf( buf, BUFFSIZE+1, "%s:%s\n", score.query[i].elemName.c_str(), score.query[i].queryTerm.c_str() );
		  }

		  // if it is not the last clause, put a blank after
		  //
		  else {
			snprintf( buf, BUFFSIZE+1, "%s:%s ", score.query[i].elemName.c_str(), score.query[i].queryTerm.c_str() );
		  }

		  send( _s, buf, strlen( buf ), 0 );
		}

		// send the score, as a string, with two decimal digits, and end with \n
		snprintf( buf, BUFFSIZE+1, "%.20f\n", score.score);
		send( _s, buf, strlen( buf ), 0 );
}
