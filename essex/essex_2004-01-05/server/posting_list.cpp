...
// the selection (field filter).
//
// We also update the {docid => { field => count}} structure for use in 
// determining field weight component of rank later.
//
// We also update the field frequency structure for this term (similar to 
// the document field counts, except we sum across all documents in the 
// postings list for this term).  This is used for structured queries.
//
intmap<short>* posting_list::get_list(int collection_size, 
	int num_tags, 
	int _tagid, 
	...
	for (it = list.begin(); it != list.end(); it++) {
...
#ifdef FF_WEIGHTING
		// only need to calc this every time if we are doing ff weighting
		int occurrences = get_count(*it);

		// update field frequencies for the term
		if (_tagid != -1) 
			(*field_frequencies)[tagid] += occurrences;
#endif

		// this "if" statement is where field filtering happens
		if (_tagid == -1 || tagid == _tagid) {
			...
			// add occurrences to standard (tf) postings list return slice
			(*r)[docid] += occurrences;
			...
			(*field_counts[docid])[tagid] += occurrences;
