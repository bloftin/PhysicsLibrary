...
bool Morphology::isvowel( char c ) {
	return c=='a'||c=='e'||c=='i'||c=='o'||c=='u';
}

string Morphology::vowelize( string word ) {
	// aeiou are vowels
...
bool Morphology::isLetter(char _c) {
	char c = toascii(_c);
	return ((c>='A' && c<='Z') || 
			(c>='a' && c<='z') ||
		    (c>='0' && c<='9') ||
...
// remove punctuation
//
// this is actually more like a "whitelist" operation-- only allow explicitly
// allowed characters (alphabetical 
string Morphology::removePunc(string word) {
	string r="";
...
bool Morphology::suffix( string const &word, string const &suffix ) {
...
int Morphology::porterm( string word ) {
...
string Morphology::stem_and_stop( string word, int add ) {
	for( int i = 0; i < word.length(); ++i )
	...
	// APK - cache this for later use
	string original_word = word;

	// look up index of word in dict hash
	int pos = index_of(word);

	// if raw word isn't in dictionary, try stemming it
	//
	if( _values[pos] == EMPTY ) {

		// get the stemmed version
		word = stem(word);
		...
		// if in add mode, add the resulting word
		// 
		if ( add ) {
			_values[pos] = word;
		} 
		
		// if in read mode only, see if stemmed word is in dictionary
		// 
		else {
			if (word == original_word) return original_word;
			
			pos = index_of(word); // get index of new word
			
			// if the word still isn't found, return the original word
			if (_values[pos] == EMPTY) return original_word;
...
string Morphology::stem_and_stop( string word) {

	return stem_and_stop(word, 1);
}

/* stem and stop helper */

string Morphology::stem(string word) {
...
/*
void Morphology::print() {

	int idx;
	for (idx=0; idx<TERM_HASH_SIZE; idx++) 
		if (_keys[idx] != "") 
			cout << "Morphology:_keys[" << idx << "] = " << _keys[idx] << ", Morphology::_values[" 
			     << idx << "] = " << _values[idx] << "\n";
}


int Morphology::writeDisk(int start_pos, fstream& file) {
...
int Morphology::readDisk(int start_pos, fstream& file) {
