  ...
  <xsl:with-param name="content">
    ...
    <xsl:call-template name="clearbox">
        ...
        <xsl:with-param name="content">
			...
			<!-- control for adding user easily to the viewer's groups -->
			
			<xsl:if test="addablegroup">

				<br /><br />

			   <form method="post" action="/">

			   Add <xsl:value-of select="username"/> to your group: 

			   <!-- build select box of group ids -->
			   <select name="groupid">
			   
			   <xsl:for-each select="addablegroup">
			   ...
			   <input type="hidden" name="op" value="addusertogroup" />
			   <input>
