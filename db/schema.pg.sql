...
/******************************************************************************
 authors table -- holds author list information for planetphysics objects
*******************************************************************************/
...
/******************************************************************************
 requests - requests for planetphysics additions 
*******************************************************************************/
...
/******************************************************************************
 objlinks (generic links between planetphysics objects) 
*******************************************************************************/
...
/******************************************************************************
 words (word name->wid lookup table)
*******************************************************************************/
create sequence words_uid_seq;
create table words(
 uid int8 default nextval('words_uid_seq'),
 word varchar(32) unique not null
);

--create index words_uid_idx on words using hash(uid);
create index words_uid_idx on words (uid);
--create index words_word_idx on words using hash(word);
create index words_word_idx on words (word);

/******************************************************************************
 wordidx (word indexing table can tell us if a word is in an entry)
*******************************************************************************/
create table wordidx (
 wid integer not null,     /* word id from dictionary */
 objectid int8 not null,   /* object id that contains the word */
 tbl varchar(16) not null  /* table the object is in */
);
--create index wordidx_wid_idx on wordidx using hash(wid);
create index wordidx_wid_idx on wordidx (wid);
--create index wordidx_objectid_idx on wordidx using hash(objectid);
