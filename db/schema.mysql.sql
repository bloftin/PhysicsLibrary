...
CREATE TABLE rendered_images (
  ...
  PRIMARY KEY  (uid),
  KEY rendered_images_imagekey_idx (imagekey)
) TYPE=InnoDB;
...
CREATE TABLE wordidx (
  ...
  objectid int(11) NOT NULL default '0',
  tbl varchar(16) NOT NULL default '',
  KEY wordidx_objectid_idx (objectid),
  KEY wordidx_wid_idx (wid)
) TYPE=InnoDB;
...
CREATE TABLE words (
  ...
  PRIMARY KEY  (uid),
  UNIQUE KEY words_word_key (word)
) TYPE=InnoDB;

--
-- Table structure for table 'words_uid_seq'
--

CREATE TABLE words_uid_seq (
  val int(11) NOT NULL auto_increment,
  PRIMARY KEY  (val)
) TYPE=MyISAM;
